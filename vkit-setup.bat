...
set "VKIT_PREBUILT_EXT_DIR=%VKIT_ROOT_DIR%\prebuilt-ext\%VKIT_DEVICE_PLATFORM%"
set "VKIT_PACKUP_DIR=%VKIT_ROOT_DIR%\packup\%VKIT_DEVICE_PLATFORM%"
set "VKIT_ETC_DIR=%VKIT_PREBUILT_DIR%\etc"
set "VKIT_CODE_COMPLETE_DIR=%VKIT_ETC_DIR%\vkit-code-complete"
...
if exist "%VKIT_ROOT_DIR%\prebuilt-ext" (
    if not exist "%VKIT_PREBUILT_EXT_DIR%" mkdir "%VKIT_PREBUILT_EXT_DIR%"
)
if not defined VKIT_PROTO_DIR (
    if exist "%VKIT_ROOT_DIR%\middleware\vlink-msgs\proto" (
        set "VKIT_PROTO_DIR=%VKIT_ROOT_DIR%\middleware\vlink-msgs\proto"
    ) else if exist "%VKIT_ROOT_DIR%\app\vlink-msgs\proto" (
        set "VKIT_PROTO_DIR=%VKIT_ROOT_DIR%\app\vlink-msgs\proto"
    ) else if exist "%VKIT_PREBUILT_DIR%\etc\vlink-msgs\proto" (
        set "VKIT_PROTO_DIR=%VKIT_PREBUILT_DIR%\etc\vlink-msgs\proto"
    )
    if defined VKIT_DEVICE (
        if exist "%VKIT_ROOT_DIR%\middleware\vlink-msgs\etc\url_remap_%VKIT_DEVICE%.json" (
            set "VLINK_URL_REMAP=%VKIT_ROOT_DIR%\middleware\vlink-msgs\etc\url_remap_%VKIT_DEVICE%.json"
        ) else if exist "%VKIT_ROOT_DIR%\app\vlink-msgs\etc\url_remap_%VKIT_DEVICE%.json" (
            set "VLINK_URL_REMAP=%VKIT_ROOT_DIR%\app\vlink-msgs\etc\url_remap_%VKIT_DEVICE%.json"
        ) else if exist "%VKIT_PREBUILT_DIR%\etc\vlink-msgs\etc\url_remap_%VKIT_DEVICE%.json" (
            set "VLINK_URL_REMAP=%VKIT_PREBUILT_DIR%\etc\vlink-msgs\etc\url_remap_%VKIT_DEVICE%.json"
        )
    )
)
echo %VKIT_PROTO_DIR%| findstr /I "vlink-msgs" >nul
if %ERRORLEVEL%==0 (
    set "VLINK_SCHEMA_PLUGIN=vlink-msgs"
)
...
if "%~1"=="" (
    ...
    for /f "tokens=1-2 delims=/:" %%i in ('time /t') do set _NOW=%%i:%%j
    echo # "Date: !_TODAY! !_NOW!"
    echo # "Copyright (C) 2026 by Thun Lu. All rights reserved."
    echo ##########################################################################
    echo.
    ...
    if /I "%~1"=="pull" (
        ...
        if exist "%VKIT_ROOT_DIR%\prebuilt-ext" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\prebuilt-ext"
        if exist "%VKIT_ROOT_DIR%\thirdparty" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\thirdparty"
        if exist "%VKIT_ROOT_DIR%\thirdparty-ext" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\thirdparty-ext"
        if exist "%VKIT_ROOT_DIR%\vendor" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\vendor"
        if exist "%VKIT_ROOT_DIR%\vendor-ext" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\vendor-ext"
        if exist "%VKIT_ROOT_DIR%\middleware" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\middleware"
        if exist "%VKIT_ROOT_DIR%\middleware-ext" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\middleware-ext"
        if exist "%VKIT_ROOT_DIR%\app" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\app"
        if exist "%VKIT_ROOT_DIR%\app-ext" set "_pull_list=!_pull_list! %VKIT_ROOT_DIR%\app-ext"
    ...
    if /I "%~1"=="rclean" (
        ...
        if exist "%VKIT_ROOT_DIR%\prebuilt-ext\.git" (
            if exist "%VKIT_PREBUILT_EXT_DIR%" (
                pushd "%VKIT_PREBUILT_EXT_DIR%"
                git clean -fdx .
                git checkout HEAD .
                popd
            ) else if exist "%VKIT_ROOT_DIR%\prebuilt-ext" (
                pushd "%VKIT_ROOT_DIR%\prebuilt-ext%"
                git checkout HEAD "%VKIT_DEVICE_PLATFORM%"
                popd
            )
        ) else (
            if exist "%VKIT_PREBUILT_EXT_DIR%" rmdir /s /q "%VKIT_PREBUILT_EXT_DIR%"
        )
        exit /b 0
    )
    if /I "%~1"=="dclean" (
    ...
    if /I "%~1"=="aclean" (
        ...
        if exist "%VKIT_ROOT_DIR%\prebuilt-ext" rmdir /s /q "%VKIT_ROOT_DIR%\prebuilt-ext"
...
set "_hay=%~1"
set "_needle=%~2"
call set "_hay2=%%_hay:~0,%=_needle:~0,0%%%"
set "_cmp=!_hay:%_needle%=!"
if not "!_cmp!"=="!_hay!" exit /b 0
exit /b 1
