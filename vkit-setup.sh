...
export VKIT_ROOT_DIR=${VKIT_ROOT_DIR:-$(cd $(dirname ${BASH_SOURCE:-$0}) && pwd)}
export VKIT_HOST_OS="$(uname -o | tr '[:upper:]' '[:lower:]')"
...
export VKIT_PREBUILT_EXT_DIR=$VKIT_ROOT_DIR/prebuilt-ext/$VKIT_DEVICE_PLATFORM
...
[ -d $VKIT_ROOT_DIR/prebuilt-ext ] && mkdir -p $VKIT_PREBUILT_EXT_DIR

if [ -z "$VKIT_PROTO_DIR" ];then
    if [ -d $VKIT_ROOT_DIR/middleware/vlink-msgs/proto ];then
        export VKIT_PROTO_DIR=$VKIT_ROOT_DIR/middleware/vlink-msgs/proto
    elif [ -d $VKIT_ROOT_DIR/app/vlink-msgs/proto ];then
        export VKIT_PROTO_DIR=$VKIT_ROOT_DIR/app/vlink-msgs/proto
    elif [ -d $VKIT_PREBUILT_DIR/etc/vlink-msgs/proto ];then
        export VKIT_PROTO_DIR=$VKIT_PREBUILT_DIR/etc/vlink-msgs/proto
    fi
    if [ -n "$VKIT_DEVICE" ];then
        if [ -f $VKIT_ROOT_DIR/middleware/vlink-msgs/etc/url_remap_${VKIT_DEVICE}.json ];then
            export VLINK_URL_REMAP=$VKIT_ROOT_DIR/middleware/vlink-msgs/etc/url_remap_${VKIT_DEVICE}.json
        elif [ -f $VKIT_ROOT_DIR/app/vlink-msgs/etc/url_remap_${VKIT_DEVICE}.json ];then
            export VLINK_URL_REMAP=$VKIT_ROOT_DIR/app/vlink-msgs/etc/url_remap_${VKIT_DEVICE}.json
        elif [ -d $VKIT_PREBUILT_DIR/etc/vlink-msgs/url_remap_${VKIT_DEVICE}.json ];then
            export VLINK_URL_REMAP=$VKIT_PREBUILT_DIR/etc/vlink-msgs/url_remap_${VKIT_DEVICE}.json
        fi
    fi
fi
if [[ "$VKIT_PROTO_DIR" == *vlink-msgs* ]];then
    export VLINK_SCHEMA_PLUGIN=vlink-msgs
fi
...
function mm() {
    ...
    if [ -f $_project_dir/CMakeLists.txt ];then
        ...
        [ "$(pwd)" = $VKIT_ROOT_DIR ] && echo -e "\033[1m\033[31mError: Can not mm project [$_project_dir]!\033[0m" && return 1
        ...
        if [ $_build_type -eq 0 ];then
            if [[ $@ == --target* ]];then
                ...
                if [ "$VKIT_STRIP" = "1" ];then
                    cmake --install $VKIT_BUILD_DIR/$_project >/dev/null --strip
                else
                    cmake --install $VKIT_BUILD_DIR/$_project >/dev/null
                fi
            fi
        elif [ $_build_type -eq 1 ];then
            mkdir -p $VKIT_BUILD_DIR/$_project && (yes | cp -rf $_project_dir/* $VKIT_BUILD_DIR/$_project)
...
function mm_thirdparty() {
    _mm_for_cfg "$VKIT_PLATFORM_CONFIG_DIR/thirdparty.cfg" $1
}

function mm_vendor() {
    _mm_for_cfg "$VKIT_PLATFORM_CONFIG_DIR/vendor.cfg" $1
}

function mm_middleware() {
    if [ "${VKIT_MIDDLEWARE_RELWITHDEBINFO}" = "1" ];then
        _mm_for_cfg "$VKIT_PLATFORM_CONFIG_DIR/middleware.cfg" $1 -DCMAKE_BUILD_TYPE="RelWithDebInfo"
    else
        _mm_for_cfg "$VKIT_PLATFORM_CONFIG_DIR/middleware.cfg" $1
    fi
}

function mm_app() {
    if [ "${VKIT_APP_RELWITHDEBINFO}" = "1" ];then
        _mm_for_cfg "$VKIT_PLATFORM_CONFIG_DIR/app.cfg" $1 -DCMAKE_BUILD_TYPE="RelWithDebInfo"
    else
        _mm_for_cfg "$VKIT_PLATFORM_CONFIG_DIR/app.cfg" $1
    fi
}
...
if [ "$1" = "import" ];then
    ...
    [ -d $VKIT_ROOT_DIR/setup ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/setup"
    [ -d $VKIT_ROOT_DIR/prebuilt ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/prebuilt"
    [ -d $VKIT_ROOT_DIR/prebuilt-ext ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/prebuilt-ext"
    [ -d $VKIT_ROOT_DIR/thirdparty ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/thirdparty"
    [ -d $VKIT_ROOT_DIR/thirdparty-ext ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/thirdparty-ext"
    [ -d $VKIT_ROOT_DIR/vendor ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/vendor"
    [ -d $VKIT_ROOT_DIR/vendor-ext ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/vendor-ext"
    [ -d $VKIT_ROOT_DIR/middleware ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/middleware"
    [ -d $VKIT_ROOT_DIR/middleware-ext ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/middleware-ext"
    [ -d $VKIT_ROOT_DIR/app ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/app"
    [ -d $VKIT_ROOT_DIR/app-ext ] && _pull_list="$_pull_list $VKIT_ROOT_DIR/app-ext"
    ...
    if [ -d $VKIT_ROOT_DIR/prebuilt-ext/.git ];then
        if [ -d $VKIT_PREBUILT_EXT_DIR ];then
            git -C $VKIT_PREBUILT_EXT_DIR clean -fdx .
            git -C $VKIT_PREBUILT_EXT_DIR checkout HEAD .
        elif [ -d $VKIT_ROOT_DIR/prebuilt-ext ];then
            git -C $VKIT_ROOT_DIR/prebuilt-ext checkout HEAD $VKIT_DEVICE_PLATFORM
        fi
    else
        [ -d $VKIT_PREBUILT_EXT_DIR ] && rm -rf $VKIT_PREBUILT_EXT_DIR/*
    fi
    exit 0
elif [ "$1" = "dclean" ];then
    [ -d $VKIT_PREBUILT_DIR ] && rm -rf $VKIT_PREBUILT_DIR/*
    ...
    [ -d $VKIT_ROOT_DIR/prebuilt-ext ] && rm -rf $VKIT_ROOT_DIR/prebuilt-ext/*
